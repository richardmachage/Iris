...
val receiveRgexPattern = Pattern.compile(
    """(\w+) Confirmed\.You have received Ksh([\d,]+\.\d{2}) from ([A-Za-z\s]+) (\d{10}) on (\d+/\d+/\d{2}) at (\d+:\d{2} (?:AM|PM))\s+ New M-PESA balance is Ksh([\d,]+\.\d{2})\. Use a unique M-PESA PIN to keep your money safe - don't use your date of birth as your PIN\."""
)
